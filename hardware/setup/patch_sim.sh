...
case $SIMULATOR in
  ...
  # Assume HBM is always enabled
    ...
    if [ -f ${SNAP_HARDWARE_ROOT}/sim/xcelium/run.f ]; then
      ...
      perl -i.ori -pe 'BEGIN{undef $/;} s/^-makelib xcelium_lib\/xilinx_vip/-makelib xcelium_lib\/work/mg' ${SNAP_HARDWARE_ROOT}/sim/xcelium/run.f; # set lib name to work in run.f
    fi
    ;;
  "questa"|"modelsim")
    sed -i "s/  simulate/# simulate/g"                   $1/$2 # run up to elaboration, skip execution
